      ...
      <header className="sticky top-0 z-20 w-full bg-surface-bg/95 backdrop-blur border-b border-surface-border safe-pt font-sans">
        <div className="flex items-center justify-between h-14 px-3 sm:px-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2 md:hidden">
              ...
              <div className="hidden xs:block">
                ...
                <p className="text-[10px] text-content-muted leading-none font-semibold">J2K Platform</p>
              </div>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center space-x-2">
            ...
            {isSlow && !isOffline && (
              <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-medium">
                ...
                <span>{t("common.slowNetwork")}</span>
              </div>
            )}

            {!isOffline && !isSlow && (
              <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-semibold">
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2">
            {/* 3-Language Switcher (Thai, Myanmar, Khmer) */}
            ...
            {(currentUser?.isSuperAdmin || currentUser?.isSecurityAdmin) && (
              <Link
                ...
                className="hidden xl:flex items-center space-x-1.5 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 px-2.5 py-1 rounded-full transition-all"
            ...
            <button
              ...
              className="hidden lg:flex items-center space-x-1.5 text-[11px] font-bold text-brand-600 dark:text-brand-400 bg-brand-500/10 hover:bg-brand-500/20 border border-brand-500/20 px-2.5 py-1 rounded-full transition-all active:scale-95"
            >
            ...
            <button
              ...
              className="p-1.5 sm:p-2 rounded-xl text-content-secondary hover:bg-surface-subtle transition-colors"
            >
              {darkMode ? <Sun className="w-4 h-4 text-amber-400" /> : <Moon className="w-4 h-4" />}
            </button>

            {/* User Profile Snippet */}
            <div className="flex items-center space-x-1.5 sm:space-x-2 pl-1.5 sm:pl-2 border-l border-surface-border">
              <div className="flex items-center space-x-2">
                ...
                {currentUser?.name && (
                  <div className="hidden xl:block text-left text-xs leading-tight">
                    ...
                    <p className="text-[10px] text-brand-600 dark:text-brand-400 font-semibold">
                      {currentUser.primaryRole || currentUser.role || "User"}
                    </p>
                  </div>
                )}
              </div>
              ...
              <button
                ...
                className="flex items-center space-x-1 px-2 py-1.5 sm:px-2.5 sm:py-1.5 rounded-xl text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 transition-colors border border-rose-500/20 active:scale-95"
              >
