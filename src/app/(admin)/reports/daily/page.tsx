...
export default function DailyReportPage() {
  ...
  const load = async () => { const response = await fetch(`/api/admin/reports/daily?date=${date}`, { cache: "no-store" }); const body = await response.json(); if (response.ok) setReport(body); else showError("โหลดรายงานไม่สำเร็จ", body.error); };
  useEffect(() => { load(); }, [date]);
