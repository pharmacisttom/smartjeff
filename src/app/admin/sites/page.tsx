  ...
  const handleOpenModal = (site?: Site) => {
    if (site) {
      ...
      setForm({
        ...
        minimumWorkforce: (site.minimumWorkforce ?? 1).toString(),
        requiresSupervisor: Boolean(site.requiresSupervisor),
      });
    } else {
      setEditingSite(null);
      setForm({
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sites.map((site) => (
            ...
            >
              ...
              {/* Employee Count Footer & Live Operations Config */}
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 text-xs font-semibold text-content-secondary border-t border-surface-border/40">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                    ขั้นต่ำ: {site.minimumWorkforce ?? 1} คน
                  </span>
                  {site.requiresSupervisor && (
                    <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800/40">
                      ★ ต้องมี Supervisor
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className="text-[11px] text-content-muted">ประจำ:</span>
                  <span className="text-brand-700 font-bold bg-brand-50 px-2.5 py-0.5 rounded-full text-xs">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-surface-card border border-surface-border rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-5 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              {/* Phase 7: Live Operations Parameters */}
              <div className="p-3 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-500/20 space-y-2">
                <span className="block font-bold text-emerald-800 dark:text-emerald-300 text-[11px]">
                  ⚙️ ตั้งค่าศูนย์บัญชาการสด (Live Operations Config)
                </span>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-medium text-content-secondary text-[11px] mb-1">
                      กำลังคนขั้นต่ำ (Minimum Workforce)
                    </label>
                    <input
                      type="number"
                      min="1"
                      value={form.minimumWorkforce}
                      onChange={(e) => setForm({ ...form, minimumWorkforce: e.target.value })}
                      placeholder="เช่น 1 หรือ 5"
                      className="w-full px-3 py-1.5 rounded-xl border border-surface-border bg-surface-bg text-content-primary outline-none text-xs"
                    />
                  </div>
                  <div className="flex flex-col justify-center">
                    <label className="flex items-center gap-2 cursor-pointer mt-3">
                      <input
                        ...
                        onChange={(e) => setForm({ ...form, requiresSupervisor: e.target.checked })}
                        className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 border-surface-border"
                      />
                      <span className="font-semibold text-content-primary text-[11px]">
                        ต้องการ Supervisor ประจำไซต์
                      </span>
                    </label>
                  </div>
                </div>
